...
namespace stack_exchange_redis.src
{
    public class SocketForPool
    {
        ...
        private Lazy<ConnectionMultiplexer> CreateConnection()
        {
            ...
            {
                string cacheConnection = "192.168.86.60:6379,password=dx4mzzmwy2yWQnWHAjJ5";
        ...
        {
            return BasicRetry(() => lazyConnection.Value.GetDatabase());
        }

        public System.Net.EndPoint[] GetEndPoints()
        {
            return BasicRetry(() => lazyConnection.Value.GetEndPoints());
        }

        public IServer GetServer(string host, int port)
        {
            return BasicRetry(() => lazyConnection.Value.GetServer(host, port));
        }
    }
}